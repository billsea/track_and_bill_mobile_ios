...
- (id)init {
  ...
  self.timerValue =
      [NSString stringWithFormat:@"%02.0f:%02.0f:%02.0f", 0.0, 0.0, 0.0];
  _TimerRunning = FALSE;
...
- (void)setSessionDate:(NSDate *)sDate {
  ...
  //			//[dateFormatter initWithDateFormat:@"%d/%m/%y"
  //allowNaturalLanguage:NO];
  //		}else{
  //            [dateFormatter setDateFormat:@"%m/%d/%y"];
  //			//[dateFormatter initWithDateFormat:@"%m/%d/%y"
  //allowNaturalLanguage:NO];
  //		}
  //
  //		NSString *modString = [[NSString alloc] initWithString:
  //[dateFormatter stringFromDate:sDate]];
...
- (int)getDateFormatSetting {
  NSString *settingsPath =
      [[NSBundle bundleWithIdentifier:@"com.loudsoftware.TrackAndBill"]
  ...
  // NSLog(@"date format is = %d",[[settingsDict objectForKey:@"dateFormat"]
  // intValue] );
...
- (void)updateClock {
  ...
  // NSLog(@"timer: %@",[NSString stringWithFormat:@"%02.0f:%02.0f:%04.1f",
